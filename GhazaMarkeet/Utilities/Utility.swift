...
@objc class Utility: NSObject {
    ...
    class func getIntroductionDesc(text: String) -> NSMutableAttributedString {
        let font = UIDevice.current.userInterfaceIdiom == .pad ? UIFont(name: "Comfortaa-Regular", size: 25.0)! : UIFont(name: "Comfortaa-Bold", size: 19.0)!
        let paragraphStyle = NSMutableParagraphStyle()
        paragraphStyle.lineSpacing = 10
        paragraphStyle.alignment = .left
        return NSMutableAttributedString(string: text, attributes: [NSAttributedString.Key.paragraphStyle: paragraphStyle, NSAttributedString.Key.font: font, NSAttributedString.Key.foregroundColor: UIColor.white])
    }
    
    class func getIntroductionOfSession(text: String) -> NSMutableAttributedString {
        let font = UIDevice.current.userInterfaceIdiom == .pad ? UIFont(name: "Comfortaa-Regular", size: 18.0)! : UIFont(name: "Comfortaa-Bold", size: 14.0)!
        let paragraphStyle = NSMutableParagraphStyle()
